...
CREATE TABLE public.loja_clientes (
  ...
  projeto_id uuid NOT NULL,
  loja_id uuid,
  nome text NOT NULL CHECK (length(TRIM(BOTH FROM nome)) > 0),
  email text,
  telefone text,
  documento text,
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  ...
  CONSTRAINT loja_clientes_pkey PRIMARY KEY (id),
  CONSTRAINT loja_clientes_projeto_id_fkey FOREIGN KEY (projeto_id) REFERENCES public.projetos(id),
  CONSTRAINT loja_clientes_loja_id_fkey FOREIGN KEY (loja_id) REFERENCES public.mercadolivre_lojas(id)
);
CREATE TABLE public.loja_estoque_reservas (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  projeto_id uuid NOT NULL,
  loja_id uuid,
  pedido_id uuid,
  mercadolivre_item_id text NOT NULL CHECK (length(TRIM(BOTH FROM mercadolivre_item_id)) > 0),
  quantidade integer NOT NULL DEFAULT 1 CHECK (quantidade > 0),
  status text NOT NULL DEFAULT 'ativa'::text CHECK (status = ANY (ARRAY['ativa'::text, 'confirmada'::text, 'liberada'::text, 'expirada'::text])),
  expires_at timestamp with time zone NOT NULL,
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT loja_estoque_reservas_pkey PRIMARY KEY (id),
  CONSTRAINT loja_estoque_reservas_projeto_id_fkey FOREIGN KEY (projeto_id) REFERENCES public.projetos(id),
  CONSTRAINT loja_estoque_reservas_loja_id_fkey FOREIGN KEY (loja_id) REFERENCES public.mercadolivre_lojas(id)
);
CREATE TABLE public.loja_pagamento_config (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  projeto_id uuid NOT NULL,
  loja_id uuid,
  provider text NOT NULL DEFAULT 'mercado_pago'::text,
  status text NOT NULL DEFAULT 'desconectado'::text CHECK (status = ANY (ARRAY['desconectado'::text, 'conectado'::text, 'erro'::text, 'pendente'::text])),
  mode text NOT NULL DEFAULT 'test'::text CHECK (mode = ANY (ARRAY['test'::text, 'production'::text])),
  public_key text,
  ...
  CONSTRAINT loja_pagamento_config_pkey PRIMARY KEY (id),
  CONSTRAINT loja_pagamento_config_projeto_id_fkey FOREIGN KEY (projeto_id) REFERENCES public.projetos(id),
  CONSTRAINT loja_pagamento_config_loja_id_fkey FOREIGN KEY (loja_id) REFERENCES public.mercadolivre_lojas(id)
);
CREATE TABLE public.loja_pedido_itens (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT loja_pedido_itens_pkey PRIMARY KEY (id),
  CONSTRAINT loja_pedido_itens_pedido_id_fkey FOREIGN KEY (pedido_id) REFERENCES public.loja_pedidos(id),
  CONSTRAINT loja_pedido_itens_projeto_id_fkey FOREIGN KEY (projeto_id) REFERENCES public.projetos(id),
  CONSTRAINT loja_pedido_itens_snapshot_product_id_fkey FOREIGN KEY (snapshot_product_id) REFERENCES public.mercadolivre_produtos_snapshot(id)
);
CREATE TABLE public.loja_pedido_pagamentos (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  pedido_id uuid,
  projeto_id uuid,
  provider text NOT NULL DEFAULT 'mercado_pago'::text,
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT loja_pedido_pagamentos_pkey PRIMARY KEY (id),
  CONSTRAINT loja_pedido_pagamentos_pedido_id_fkey FOREIGN KEY (pedido_id) REFERENCES public.loja_pedidos(id),
  CONSTRAINT loja_pedido_pagamentos_projeto_id_fkey FOREIGN KEY (projeto_id) REFERENCES public.projetos(id)
);
CREATE TABLE public.loja_pedidos (
  ...
  loja_id uuid,
  cliente_id uuid,
  status text NOT NULL DEFAULT 'rascunho'::text CHECK (status = ANY (ARRAY['rascunho'::text, 'aguardando_pagamento'::text, 'pago'::text, 'cancelado'::text, 'expirado'::text, 'falhou'::text])),
  payment_status text NOT NULL DEFAULT 'pendente'::text CHECK (payment_status = ANY (ARRAY['pendente'::text, 'aprovado'::text, 'em_analise'::text, 'recusado'::text, 'cancelado'::text, 'estornado'::text])),
  fulfillment_status text NOT NULL DEFAULT 'pendente'::text CHECK (fulfillment_status = ANY (ARRAY['pendente'::text, 'separando'::text, 'enviado'::text, 'entregue'::text, 'cancelado'::text])),
  currency_id text NOT NULL DEFAULT 'BRL'::text,
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT loja_pedidos_pkey PRIMARY KEY (id),
  CONSTRAINT loja_pedidos_projeto_id_fkey FOREIGN KEY (projeto_id) REFERENCES public.projetos(id),
  CONSTRAINT loja_pedidos_loja_id_fkey FOREIGN KEY (loja_id) REFERENCES public.mercadolivre_lojas(id),
  CONSTRAINT loja_pedidos_cliente_id_fkey FOREIGN KEY (cliente_id) REFERENCES public.loja_clientes(id)
);
