...
import { deleteMercadoLivreConnectorForUser, upsertMercadoLivreConnectorForUser } from "@/lib/mercado-livre-connector"
...
export async function DELETE(request, context) {
  ...
  const body = await request.json().catch(() => ({}))
  if (String(body.provider || "").trim().toLowerCase() !== "mercado_livre") {
    return NextResponse.json({ error: "Provider de conector inválido." }, { status: 400 })
  }

  const { ok, error } = await deleteMercadoLivreConnectorForUser(project, body, user)
