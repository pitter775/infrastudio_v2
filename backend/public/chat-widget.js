(function () {
  ...
  ready(async function () {
    ...
    style.textContent = [
      ...
      ".chat-panel { position: absolute; right: 8px; bottom: 68px; width: min(380px, calc(100vw - 32px)); height: min(620px, calc(100vh - 110px)); height: min(620px, calc(100dvh - 110px)); height: min(620px, calc(var(--viewport-height, 100dvh) - 110px)); display: flex; pointer-events: none; flex-direction: column; overflow: hidden; border-radius: 16px; border: 0; background: " + panelBackground + "; color: " + panelText + "; box-shadow: " + (theme === "light" ? "0 0 0 5px " + panelFrame : "0 0 0 5px rgba(0,0,0,0.22)") + "; backdrop-filter: none; opacity: 0; visibility: hidden; transform-origin: calc(100% - 30px) calc(100% + 38px); transform: translate(18px, 30px) scale(.18); transition: width .18s ease, height .18s ease, visibility 0s linear .5s; }",
      ".chat-wrap.is-expanded .chat-panel { width: min(680px, calc(100vw - 32px)); height: calc(var(--viewport-height, 100dvh) - 104px); right: 8px; bottom: 64px; }",
      ".chat-panel.open { pointer-events: auto; visibility: visible; opacity: 1; transform: translate(0, 0) scale(1); animation: chatPanelEnter .58s cubic-bezier(.2,.9,.2,1) both; transition-delay: 0s; }",
      ".chat-panel.closing { pointer-events: none; visibility: visible; animation: chatPanelExit .34s cubic-bezier(.45,0,.2,1) both; transition-delay: 0s; }",
      ...
      ".chat-stack { display: flex; flex-direction: column; align-items: flex-start; gap: 12px; transition: opacity .16s ease, transform .16s ease; }",
      ".chat-stack.is-resetting { opacity: 0; transform: translateY(6px); }",
      ".chat-bubble { display: block; width: fit-content; max-width: 86%; border-radius: 14px; border: 0; padding: 11px 13px; font-size: 13px; line-height: 1.58; animation: chatBubbleIn .22s ease both; }",
      ...
      ".chat-bubble.user { max-width: 78%; align-self: flex-end; margin-left: auto; background: " + userBubbleBg + "; color: #ffffff; border-radius: 9px; border-bottom-right-radius: 4px; box-shadow: inset 0 1px 0 rgba(255,255,255,0.03), 0 2px 4px rgba(0,0,0,0.28); backdrop-filter: none; padding: 10px 12px; }",
      ...
      ".chat-ui-cards .chat-ui-card { flex: 0 0 min(214px, 72%); scroll-snap-align: start; }",
...
".chat-bubble.ai .chat-cta:active { transform: scale(0.96); box-shadow: 0 4px 12px color-mix(in srgb, " + accent + " 30%, transparent), inset 0 2px 4px rgba(0,0,0,0.25); }",
      ...
      ".chat-day-divider { display: flex; align-self: stretch; width: 100%; align-items: center; gap: 10px; margin: 4px 0; color: rgba(148,163,184,0.72); }",
      ...
      ".chat-product-carousel-nav { position: absolute; top: -34px; z-index: 2; display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; margin-top: 0; border: 0; border-radius: 999px; background: transparent; color: #39f256; box-shadow: none; cursor: pointer; transition: transform .18s ease, opacity .18s ease; }",
      ".chat-product-carousel-nav:hover { transform: translateY(-1px) scale(1.03); background: transparent; }",
      ".chat-product-carousel-nav.prev { left: auto; right: 42px; }",
      ".chat-product-carousel-nav.next { right: 10px; }",
      ...
      ".chat-product-meta { display: block; }",
      ".chat-product-click { color: inherit; text-decoration: none; cursor: pointer; }",
      ".chat-product-click:hover .chat-asset-title { color: " + accent + "; }",
      ".chat-product-price { display: block; width: 100%; margin-top: 6px; color: " + accent + "; text-decoration: none; }",
      ".chat-product-price-main { display: block; font-size: 13px; line-height: 1.15; font-weight: 500; }",
      ".chat-product-installment { display: block; margin-top: 2px; font-size: 9px; line-height: 1.2; font-weight: 500; color: " + (theme === "light" ? "#475569" : "rgba(203,213,225,0.86)") + "; }",
      ".chat-asset-body { padding: 9px; }",
      ".chat-asset-actions { margin-top: 8px; display: flex; align-items: center; gap: 6px; }",
        ...
        if (isProductAsset) {
          ...
          if (!productRail) {
            ...
            if (carouselKey) {
              ...
              productRail.scrollLeft = Number(productCarouselScrollPositions[carouselKey] || 0);
              addListener(productRail, "scroll", function () {
                productCarouselScrollPositions[carouselKey] = productRail.scrollLeft;
              }, { passive: true });
              window.requestAnimationFrame(function () {
                productRail.scrollLeft = Number(productCarouselScrollPositions[carouselKey] || 0);
              });
    ...
    function createProductAssetCard(asset, options) {
      ...
      var media = createProductAssetMedia(asset, { compact: settings.compact === true });
      if (detailUrl) {
        var mediaLink = document.createElement("a");
        mediaLink.className = "chat-product-click";
        mediaLink.href = detailUrl;
        mediaLink.target = "_blank";
        mediaLink.rel = "noreferrer noopener";
        mediaLink.setAttribute("aria-label", "Abrir detalhes do produto");
      ...
      var meta = document.createElement("div");
      meta.className = "chat-asset-meta chat-product-meta";

      var textWrap = detailUrl ? document.createElement("a") : document.createElement("div");
      textWrap.className = "chat-product-click";
      if (detailUrl) {
        textWrap.href = detailUrl;
        textWrap.target = "_blank";
        textWrap.rel = "noreferrer noopener";
      }
      var title = document.createElement("div");
      title.className = "chat-asset-title";
      ...
      if (detailUrl) {
        priceWrap.href = detailUrl;
        priceWrap.target = "_blank";
        priceWrap.rel = "noreferrer noopener";
      }
      ...
      if (installmentLabel) {
        ...
        installment.textContent = installmentLabel;
        priceWrap.appendChild(installment);
      }
    ...
    function formatApiDiagnosticsTitle(diagnostics) {
      ...
      return [
        ...
        "ApresentaÃ§Ã£o: " + (apiDiagnostics.selectedPresentation || selectedApi && selectedApi.presentation || "-"),
        "Formato: " + (apiDiagnostics.selectedResponseShape || selectedApi && selectedApi.responseShape || "-"),
        "Status: " + (apiDiagnostics.selectedStatus != null ? String(apiDiagnostics.selectedStatus) : selectedApi && selectedApi.status != null ? String(selectedApi.status) : "-"),
        "Campos: " + (apiDiagnostics.selectedFieldCount != null ? String(apiDiagnostics.selectedFieldCount) : selectedApi && selectedApi.fields != null ? String(selectedApi.fields) : "0"),
        "Itens: " + (apiDiagnostics.selectedRuntimeItemCount != null ? String(apiDiagnostics.selectedRuntimeItemCount) : selectedApi && selectedApi.runtimeItems != null ? String(selectedApi.runtimeItems) : "0"),
        "Confiança: " + (apiDiagnostics.semanticConfidence != null ? String(apiDiagnostics.semanticConfidence) : "-"),
        "API selecionada: " + (selectedApi ? selectedApi.nome || selectedApi.name || selectedApi.id : apiDiagnostics.selectedApiId || "-"),
