...
function buildLlmApiGuidePrompt(form, { urlPathParams = [], responseValue = "" } = {}) {
  ...
  return [
    "Contrato de cadastro de APIs do InfraStudio.",
    "Use este contrato como fonte de verdade para orientar o cadastro da API abaixo no painel.",
    "Não invente opções fora deste contrato. Se algum dado da API faltar, peça o dado exato que falta.",
    "",
    "Abas do painel:",
    "- Body: define dados que o agente pode enviar para a API. Em GET, normalmente serve para mapear campos/variáveis de teste; em POST/PUT/PATCH, define o corpo enviado.",
    ...
    "- Consulta por identificador: use somente quando existe identificador exato, como id, propertyId, código, protocolo, documento, uuid ou slug exato. Não use para nome, título, bairro ou termo aberto.",
    "- Busca informativa: use para FAQ, documentação, regras, políticas, base de conhecimento ou informações gerais que não retornam um item de catálogo.",
    "- Busca de catálogo: use quando o cliente procura itens por nome, título, termo, categoria, bairro, cidade, condomínio, característica, produto ou imóvel. É o tipo correto para URLs como /busca?titulo={titulo}.",
    "- Cadastro / envio de dados: use para criar lead, pedido, solicitação, cadastro, reserva ou qualquer operação com efeito colateral.",
    "",
    "Escopos de uso:",
    "- Sempre disponível: a API pode ser considerada em qualquer conversa. Use com cuidado porque aumenta chance de conflito.",
    "- Busca aberta: use na home, atendimento aberto ou vitrine, quando o cliente ainda não está preso a um item específico. Exemplo: buscar imóveis por título.",
    "- Item atual: use quando o chat já recebe id/propertyId/contexto de um item específico. Exemplo: /imoveis/{id}. Não deve aparecer em busca aberta por nome.",
    "",
    "Autoexecução e confirmação:",
    "- Autoexecuta ligado: permite o agente chamar a API automaticamente quando a intenção e os campos obrigatórios estiverem resolvidos.",
    "- Autoexecuta desligado: a API pode ser conhecida pelo agente, mas não deve executar automaticamente.",
    "- Sem confirmação: adequado para GET/HEAD e consultas sem efeito colateral.",
    "- Exige confirmação: obrigatório para cadastro/envio de dados, DELETE e operações que alteram dados, cobram, disparam mensagens ou criam registros.",
    "- APIs POST/PUT/PATCH/DELETE com efeito colateral devem exigir confirmação, mesmo que tecnicamente possam autoexecutar.",
    "",
    "Body e campos obrigatórios:",
    "- Aba Campos: melhor para usuário preencher parâmetros estruturados sem escrever JSON manual.",
    "- Aba JSON: melhor quando a API exige um payload fixo, aninhado ou mais técnico.",
    "- Campo obrigatório: se marcado, o agente deve coletar esse dado antes de executar.",
    "- Em GET com variáveis na URL, cadastre campos com o mesmo nome das variáveis quando o agente precisar extrair da fala. Exemplo: URL com {titulo} deve ter campo titulo.",
    "- Em consulta por identificador, use id/propertyId/codigo como campo obrigatório.",
    "- Em busca de catálogo, use titulo/query/q/termo/busca/nome conforme a URL exigir.",
    "",
    "Response Path, Preview Path e campos de resposta:",
    "- Response Path aponta para a raiz útil do JSON retornado. Exemplo: se a resposta é { imoveis: [...] }, use imoveis para tratar a lista.",
    "- Preview Path aponta para um campo curto usado como prévia/identificação. Exemplo: titulo, summary, id ou 0.titulo quando a raiz é lista.",
    ...
    "- API por id/propertyId deve ser Consulta por identificador + Item atual.",
    "- API por nome/título/termo deve ser Busca de catálogo + Busca aberta.",
    "- A descrição para decisão da IA deve conter regras negativas claras. Exemplo: Não use esta API quando o cliente informar id ou propertyId.",
    "- Evite deixar várias APIs ativas com Sempre disponível e descrições parecidas.",
    "- Priorize até 4 APIs ativas vinculadas ao agente; deixe as mais importantes e menos ambíguas.",
    "",
    "Política de teste:",
    "- Botão Send testa a URL e substitui variáveis da API usando o campo de variáveis de teste.",
    "- O campo de variáveis do botão Send não altera o contexto real do chat.",
