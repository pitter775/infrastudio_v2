    ...
    >
      ...
      <div
        className={cn(
          ...
          active && 'border-violet-400/35 shadow-[0_0_0_1px_rgba(168,85,247,0.16),0_0_24px_rgba(168,85,247,0.18)]',
        )}
      >
        <div className="border-b border-white/5 p-5">
          <div className="flex items-center justify-between gap-3">
            <h3 className="truncate font-medium text-white">Agente: {project.name}</h3>
            {loading ? (
        ...
        <div className="flex items-center justify-between border-t border-white/5 p-4 text-[11px] font-medium text-slate-500">
          ...
          <div className="truncate">{project.type}</div>
        </div>
      </div>
    </motion.article>
  )
