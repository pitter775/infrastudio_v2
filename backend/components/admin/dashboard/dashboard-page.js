...
export function AdminDashboardPage({ overview, currentUser }) {
  ...
  return (
    <div>
      <AdminPageHeader
        title="Dashboard"
        description="Leitura consolidada do admin baseada nos dados reais do v2, inspirada no dashboard do legado."
        actions={
      ...
      <div className="mb-6 rounded-2xl border border-white/5 bg-[#0b1120] p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-4 text-2xl font-semibold text-white">Resumo operacional</h2>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3">
              ...
              <div className="mt-1 text-sm font-semibold text-white">{overview.role === "admin" ? "Admin" : "Usuario"}</div>
            </div>
            <div className="rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3">
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {summary.cards.map((card) => (
          <div key={card.label} className="rounded-xl border border-white/5 bg-[#0b1120] p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-slate-500">{card.label}</div>
            <div className="mt-2 text-2xl font-semibold text-white">{formatInteger(card.value)}</div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_420px]">
        <section className="rounded-xl border border-white/5 bg-[#0b1120] p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold text-white">Top projetos por consumo</h3>
              <p className="mt-1 text-xs text-slate-500">Leitura do ciclo atual de billing, como no legado.</p>
            </div>
